...
//Given 2 straight line segments, compute the point of intersection, if any.
class Point {
    var x: Double
    var y: Double
...
//check if middle is between start and end pointwise
func isBetween(start: Point, middle: Point, end: Point) -> Bool {
    return isBetween(start: start.x, middle: middle.x, end: end.x) && isBetween(start: start.y, middle: middle.y, end: end.y)
}


//rearrangin these so that, in order of x values: start is before end and
...
func intersection(start1:Point, end1:Point, start2: Point, end2:Point) -> Point? {
    ...
    //rearrage them so the lower values are in the start
    if start1.x > end1.x {
    ...
    //check for the slope and yIntercept
